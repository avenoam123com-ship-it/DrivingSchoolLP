...
export default function Nav() {
  ...
  const bg = useTransform(scrollY, [0, 60], ['rgba(10,10,10,0)', 'rgba(10,10,10,0.95)']);
  const blur = useTransform(scrollY, [0, 60], ['blur(0px)', 'blur(14px)']);
  const borderColor = useTransform(scrollY, [0, 60], ['rgba(255,255,255,0)', 'rgba(255,255,255,0.08)']);

  return (
    <motion.header
      style={{ background: bg, backdropFilter: blur, borderBottomColor: borderColor }}
    ...
    >
      <div className="max-w-5xl mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center gap-6" style={{ fontSize: '0.88rem' }}>
          <a href="#about"   style={{ color: '#71717a', textDecoration: 'none' }} className="hover:text-white transition-colors">אודות</a>
          <a href="#why"     style={{ color: '#71717a', textDecoration: 'none' }} className="hover:text-white transition-colors">למה יוגב</a>
          <a href="#reviews" style={{ color: '#71717a', textDecoration: 'none' }} className="hover:text-white transition-colors">המלצות</a>
        </nav>
